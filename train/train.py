...
from torch.nn import L1Loss, MSELoss, CrossEntropyLoss, BCELoss
...
def prep_for_training(args, num_train_optimization_steps: int):
    ...
    optimizer_grouped_parameters = [
        {
            "params": [
                 p for n, p in param_optimizer if not any(nd in n for nd in no_decay) and not any(nd in n for nd in CE_params) #and not any(nd in n for nd in CA_params)
            ],
            "weight_decay": args.weight_decay,
        },
        {"params": model.roberta.encoder.CE.parameters(), 'lr':args.learning_rate, "weight_decay": args.weight_decay},
        # {"params": model.roberta.encoder.CAmodule.parameters(), 'lr': args.learning_rate, "weight_decay": args.weight_decay},
        {
            "params": [
                p for n, p in param_optimizer if any(nd in n for nd in no_decay) and not any(nd in n for nd in CE_params) #and not any(nd in n for nd in CA_params)
            ],
            "weight_decay": 0.0,
        },
    ]
...
def train_epoch(args, model: nn.Module, train_dataloader: DataLoader, optimizer, scheduler):
    ...
    for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
        ...
        # loss = loss_fct(torch.max(logits, dim=-1).values, label_ids)
...
def evaluate_epoch(args, model: nn.Module, dataloader: DataLoader):
    # 加载模型
    model.load_state_dict(torch.load('MCFNet_noSTEM.pth'), False)
    ###
    model.eval()
    preds = []
    labels = []
...
def train(
    ...
    for epoch_i in range(int(args.n_epochs)):
        ...
        if best_loss > valid_loss:
            ...
            print('save model')
            torch.save(model.state_dict(), 'MCFNet_noSTEM.pth')
        ###
...
def main():
    args = parser_args()
    wandb.init(project="CENew", reinit=True)
